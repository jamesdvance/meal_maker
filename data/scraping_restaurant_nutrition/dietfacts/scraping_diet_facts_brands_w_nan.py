...
def parse_brand_pages(url):
    ...
    if (re.search(r'itemid=', url) != None) or (rest_soup.find('table', attrs={'width':'300'})!=None):
...
def parse_all_restaurants(start_url):
    ...
    #for i in range(len(ff_links_full)-1):
        if zero_finder(ff_links_full[i]):
            ...
            if i> 0:
                page_df.to_csv('restaurant_foods_in_progress_nan.csv', mode='a',index=False, header=False, index_label=0)
            else:
                page_df.to_csv('restaurant_foods_in_progress_nan.csv', index=False)
...
def scrape_all_diet_facts(start_url):
    # Getting list of all links to scrape
    ...
    for a_link in a_links:
        links_vec.append(base_url+a_link['href'])
    # Cycling through each link 
    i=0
    for link in links_vec:
        link_page = requests.get(links)
        link_soup = BeautifulSoup(link_page.text, 'html.parser')
        soup_nobr = link_soup.findAll('nobr')
        brand_list = []
        for list_item in soup_nobr:
            brand_name = list_item.find('a').text
            brand_link = base_url+list_item.find('a')['href']
            if zero_finder(brand_link):
                page_df = parse_brand_pages(brand_link)
                page_df['brand_label'] = brand_name
                page_df['category'] = 'brand'
                if i > 0:
                    page_df.to_csv('diet_facts_brand_foods_in_progress.csv', mode='a',index=False, header=False, index_label=0)
                else:
                    page_df.to_csv('diet_facts_brand_foods_in_progress.csv', index=False)
            else:
                pass
            i+=1
    return 'done'
...
brand_df = scrape_all_diet_facts('http://www.dietfacts.com/list.asp')
