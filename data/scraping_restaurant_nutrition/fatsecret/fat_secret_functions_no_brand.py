...
def parse_food_page(url, food_type, page_brand):
    ...
    for row in rows:
        if (row.find('td', attrs={'class':'borderBottom'}).find('a',attrs={'class':'brand'})==None) and (row.find('td', attrs={'class':'borderBottom'})!=None):
            ...
            food_df = pd.DataFrame([{'food':food, 'brand':np.nan, 'food_type':food_type}])
            try:
...
def parse_food_pages(url,food_type, page_brand):
    ...
    for i in range(total_pages):
        ...
        if glob_iter == 0:
            page_df.to_csv('fat_secret_no_brands_in_progress.csv', index=False)
        else:
            page_df.to_csv('fat_secret__no_brands_in_progress.csv', mode='a',index=False, header=False, index_label=0)
        glob_iter +=1
    return 'done' 
