...
def parse_food_pages(url,food_type, page_brand):
    ...
    if search_summary != None:
        ...
        for i in range(total_pages):
            ...
            if glob_iter == 0:
                page_df.to_csv('fat_secret_restaurant_in_progress.csv', index=False)
            else:
                page_df.to_csv('fat_secret_restaurant_in_progress.csv', mode='a',index=False, header=False, index_label=0)
            glob_iter +=1
    return 'done' 
